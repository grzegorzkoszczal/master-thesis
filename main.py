...
class Config:
    ...
    implementations["CPUs"] = ["OMP-CPP"]
    # implementations["GPUs"] = ["OMP-CUDA"]
    # implementations["Hybrid"] = ["OMP-CPP+OMP-CUDA"]

    benchmarks_cpu = {}
    benchmarks_gpu = {}
    # benchmarks_cpu["OMP-CPP"] = ["lu.C", "is.D", "bt.C"]
    benchmarks_cpu["OMP-CPP"] = ["lu.C"]
    # benchmarks_cpu["MPI-Fortran"] = ["apk1.CLASS", "apk2.CLASS", "apk3.CLASS"]

    # benchmarks_gpu["OMP-CUDA"] = ["ep.D", "sp.D", "lu.D"]
    benchmarks_gpu["Horovod-Python"] = ["Xception"]

    # benchmarks_cpu["OMP-CPP+OMP-CUDA"] = ["lu.C", "is.D", "bt.C"]
    # benchmarks_gpu["OMP-CPP+OMP-CUDA"] = ["ep.D", "sp.D", "lu.D"]

    # benchmarks_cpu["MPI-Fortran+Horovod-Python"] = ["apk1.CLASS", "apk2.CLASS", "apk3.CLASS"]
    # benchmarks_gpu["MPI-Fortran+Horovod-Python"] = ["Xception", "Xception", "Xception"]


    configurations_cpu = {}
    configurations_cpu["OMP-CPP"] = [
            # "1_CPU___1_Thread",
            # "1_CPU___5_Threads",
            # "1_CPU___10_Threads",
            # "1_CPU___20_Threads",
    ...
    configurations_cpu["OMP-CPP+OMP-CUDA"] = [
        "1_CPU___4_Threads",
        "1_CPU___8_Threads",
        "2_CPUs___16_Threads",
        "2_CPUs___32_Threads"
    ...
    configurations_gpu["OMP-CPP+OMP-CUDA"] = [
        "1_GPU___1_Thread",
        "2_GPUs___2_Threads",
        "4_GPUs___4_Threads",
        "8_GPUs___8_Threads"
    ...
    taskset_cpu = {
        "1_CPU___1_Thread": "0",
        "1_CPU___5_Threads": "0-4",
        "1_CPU___10_Threads": "0-9",
        "1_CPU___20_Threads": "0-9,20-29",
        "1_CPU___40_Threads": "0-19,40-59",
        "2_CPUs___2_Threads": "0,10",
        "2_CPUs___10_Threads": "0-4,10-14",
        "2_CPUs___20_Threads": "0-19",
        "2_CPUs___40_Threads": "0-39",
        "2_CPUs___80_Threads": "0-79",
    }

    # Used both GPU-only and Hybrid configs
    taskset_gpu = {}
    taskset_gpu["1_GPU___1_Thread"] = (0,)
    taskset_gpu["2_GPUs___2_Threads"] = (0, 1)
    taskset_gpu["4_GPUs___4_Threads"] = (0, 1, 10, 11)
    taskset_gpu["8_GPUs___8_Threads"] = (0, 1, 2, 3, 10, 11, 12, 13)

    # Only used in Hybrid config for CPUs
    taskset_hybrid = {
        "1_CPU___4_Threads": "1-4",
        "1_CPU___8_Threads": "2-9",
        "2_CPUs___16_Threads": "2-9,12-19",
        "2_CPUs___32_Threads": "4-9,14-39"
    }

    # paths to directories
    OMP_dir = "/home/macierz/s175405/MasterThesis/benchmarks/OMP-CPP/"
    DNN_dir = "/home/macierz/s175405/MasterThesis/source/Horovod-Python/"
...
class Benchmark:
    ...
    def perf(
        ...
        try:
            ...
            list_of_perf_pids = []
            pin_to_cpus = ("0", "10") #, "0,10")
            idx_names = {"0": "0", "10": "1", "0,10": "s"}
...
class Execution:
    ...
    def measurements_daemons_cleanup(
        ...
        if run_perf == True:
            ...
            perf_pid_1 = int(splitted[1])
            # perf_pid_2 = int(splitted[2])
            os.kill(perf_pid_0, signal.SIGINT)
            os.kill(perf_pid_1, signal.SIGINT)
            # os.kill(perf_pid_2, signal.SIGINT)
        if run_nvml == True:
            ...
            for b in range(0, benchmark_tmp, 1):
                ...
                for c in range(0, configuration_tmp, 1):
                    ...
                    for iterator in range(8, number_of_tests + 1, 1):
                        ...
                        print(benchmark_cpu)
                        print(benchmark_gpu)
                        print(configuration_cpu)
                        print(configuration_gpu)
                        print(run_yoko)
                        print(run_perf)
                        print(run_nvml)
...
# Workflow:
#   CPUs
#       if CPUs, then OMP-CPP
#           if OMP-CPP, then cg.B
#               if OMP-CPP, then 1_CPU___1_Thread
#               if OMP-CPP, then 1_CPU___5_Threads
#               ...
#               if OMP-CPP, then 2_CPUs___40_Threads
#           if OMP-CPP, then is.D
#               if OMP-CPP, then 1_CPU___1_Thread
#               if OMP-CPP, then 1_CPU___5_Threads
#               ...
#               if OMP-CPP, then 2_CPUs___40_Threads
#       if GPUs, then OMP-CUDA
#           if OMP-CUDA, then ep.D
#               if OMP-CUDA, then 4_GPUs___4_Threads
#           if OMP-CUDA, then sp.D
#               if OMP-CUDA, then 4_GPUs___4_Threads
